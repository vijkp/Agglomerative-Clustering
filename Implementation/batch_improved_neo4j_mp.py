...
from cluster_dbms import nQueryResult as nqr
import time
...
def get_query_string(bfs_node, flag=False):
    result = "START b=node:community('id:"+str(bfs_node) + "') MATCH b-[:KNOWS]-a return a"
    #result = "MATCH (a)-[:KNOWS]-(b) WHERE b.id="+"\""+str(bfs_node)+"\""+" RETURN a"
    if flag:
        print "appending {}".format(bfs_node)
    return result
...
def process_results():
    ...
    while True:
        ...
        count += 1
        bfs_node_result = result.node
        #print "processing results of node {}".format(bfs_node_result)
...
def main():
    ...
    process_thread = Thread(target = process_results)
    process_thread.daemon = True
    process_thread.start()
    print "{}: process continuing".format(__name__)
    ...
    while True:
        ...
        if (count%10 == 0) or (bfs_queue.empty() == True):
            ...
            for each_result in result:
                ...
                result_queue.put(nqr(bfs_node_result,each_result))
            batch.clear()
